...
from untangling.tracer_knot_detect.tracer import Tracer, TraceEnd, AnalyticTracer # Uncomment for triton4
...
class TracerKnotDetector():
    def __init__(self, parallel=False, analytic=False):
        self.img = None
        self.pixels = []
        ...
        self.uo_model.load_state_dict(torch.load('/home/mallika/triton4-lip/cable-untangling/untangling/tracer_knot_detect/models/under_over/uo_model.pth', map_location=device)) # Uncomment for triton4
        ...
        self.tracer = Tracer() if not analytic else AnalyticTracer()
    ...
    def _set_data(self, img, starting_pixels):
        ...
        print("Starting pixels TKD:", starting_pixels)
        self.starting_pixels_for_trace = self.interpolate_trace(starting_pixels)
        print("Starting pixels for trace TKD:", self.starting_pixels_for_trace)
    ...
    def _visualize_full(self, path=None):
        img = self.img.copy()
    ...
    def _visualize_single_color_paths(self, pixel_paths,path=None):
        img = self.img.copy()
        # file_name = f'full_img_{self.vis_idx}'
        # colors = [[(0,255, 0), (0, 128,  0)], [(255, 0, 255), (222, 202, 234)], [(255, 165, 0), (255, 255, 0)]]
        colors = [[(0,255, 0), (196, 238, 160)], [(255, 0, 255), (222, 202, 234)], [(255, 165, 0), (117, 235, 236)]]
        print("pixel paths len", len(pixel_paths))
        for i in range(len(pixel_paths)):
            pixel_path = pixel_paths[i]
            color1, color2 = colors[i]
            img = self.tracer.visualize_path_single_color(img, pixel_path, color1, color2)

        if path is None:
            cv2.imwrite('multi_cable_tracer.png', img)
        else:
            cv2.imwrite(path, img)

    def _visualize_knot(self, path=None):
        if not self.knot:
    ...
    def _find_knot(self, cancel=True):
        return self.detector.find_knot(cancel=cancel)
    ...
    def trace_and_detect_knot(self, endpoints=None, cancel=True, trace_only=False, do_perturbations=False):
        # Flow (for debugging):
        # Get trace -> 
        ...
        print("Starting pixels for trace:", self.starting_pixels_for_trace)
        # add randint noise from -2 to 2 to starting pixels
        starting_pixels_to_use = self.starting_pixels_for_trace.copy()
        if do_perturbations:
            starting_pixels_to_use += np.random.randint(-2, 2, size=starting_pixels_to_use.shape)
        self.pixels, self.trace_end = self.tracer.trace(self.img, starting_pixels_to_use, endpoints=endpoints, viz=True, path_len=500)
        ...
        for i, crossing_loc in enumerate(self.crossing_locs):
            ...
            if crop is None:
                ...
                knot_output = self._find_knot(cancel=cancel)
            ...
            uo_model_input = self._getuoitem(uo_data, i)

            # predict UON on input
            uo, prob = self._predict_uo(uo_model_input, file_name = f'uo_model_input_{i}.png')
    ...
    def perception_pipeline(self, endpoints=None, idx=0, viz=False, do_perturbations=False, vis_dir=None):
        best_output, best_end = {}, None
        for _ in range(10 if do_perturbations else 1):
            print("tracing...")
            self.vis_idx += 1
            self.trace_and_detect_knot(endpoints=endpoints, do_perturbations=do_perturbations)
            if vis_dir is not None:
                self.output_vis_dir = vis_dir
            if viz:
            ...
            #note: commented out for knot tying since not needed
            ...
            # output['trace_incomplete'] = False #not ((self.trace_end == TraceEnd.EDGE) or not self.tracer._is_uncovered_area_touching_before_idx(self.img, self.pixels, cage_idx, endpoints)) #TODO(@Kaushik): change to true as needed
            # print(output)

            # if best_end is None or self.trace_end == TraceEnd.ENDPOINT or self.trace_end == TraceEnd.FINISHED:
            #     best_output = output
            #     best_end = self.trace_end
