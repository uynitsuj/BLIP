...
class KnotDetector:
    ...
    def run_crossing_correction(self): 
        ...
        #     print(str(crossing['ID']), str(round(crossing['confidence'], 5)))
    ...
    def find_knot(self, cancel=True):
        self.run_crossing_correction()
        self.crossings_stack = self.crossings[:]
        if cancel:
            self.run_crossing_cancellation()
        self.knot = self.find_knot_helper(cancel=cancel)
        return self.knot
    
    def find_knot_helper(self, cancel=True):
        if cancel and len(self.crossings_stack) < 3:
            return []
        ...
        for cross_idx in range(len(self.crossings_stack)):
            ...
            # print('checking', cross_idx, next_cross_idx)
            # check if no occurrence of this crossing is found later
            if next_cross_idx == -1:
                continue
            
            # check if each crossing in between is paired off
            if cancel and self.check_if_all_pairs(self.crossings_stack[cross_idx + 1:next_cross_idx]):
                continue

            # check if all intermediate crossings are undercrossings
            if cancel and (all([intermediate_crossing['ID'] == 0 for intermediate_crossing in self.crossings_stack[cross_idx + 1:next_cross_idx]])):
                continue
