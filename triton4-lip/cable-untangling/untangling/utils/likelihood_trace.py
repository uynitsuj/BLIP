...
def score_path(color_img, depth_img, points, partial_paths=False):
    ...
    if not partial_paths:
        ...
        logger.debug("Max distance:", max_distance, "at", white_pixels[0][argmax_distance], white_pixels[1][argmax_distance])
...
def pixel_to_dist_from_nearest_black_point(image):
    ...
    #         # logger.debug(distances.shape, q[i].shape, distances[tuple(q[i])])
                ...
                if is_valid_successor(pt, cur_candidates[i], depth_img,
                    ...
                    if not sim_to_existing:
                        if tuple(cur_candidates[i].astype(int)) not in filtered_candidates_set:
                            ...
                            if tier == 2:
                                logger.debug("Using tier 2")
                        if len(filtered_candidates) >= (3 if not lax else 1):
                            return filtered_candidates              
    # logger.debug("went through lax", len(filtered_candidates))
    return filtered_candidates
...
def step_path(image, start_point, points_explored, points_explored_set, black_pixel_distances):
    ...
    if cur_tpl_key in step_cache:
        ...
        # if len(ret_list) > 0:
        #     step_path_time_sum += time.time()
        #     logger.debug("Cache hit", ret_list)
    ...
    # logger.debug(base_angle, angle_thresh)
...
def visualize_path(img, path, black=False, const_color=None):
...
def is_too_similar(new_path, existing_paths):
    ...
    def length_index(lst, lns, lst_cumsum=None):
        ...
        # logger.debug(lst[i - 1].shape, pcts.shape)
...
def explore_paths(image, start_point_1, start_point_2, stop_when_crossing=False):
    ...
    logger.debug("Done doing Dijkstra to find the black point distances")
    finished_paths = []
    ...
    while len(active_paths) > 0:
        iter += 1
        if iter % 100 == 0:
            logger.debug(iter, len(active_paths))
        # if iter > 120:
        #     # logger.debug(i, len(active_paths), len(active_paths[i][0]))
...
if __name__ == "__main__":
    ...
    path, paths = explore_paths(img, start_point_1, start_point_2, stop_when_crossing=True)
    
    if path is not None:
        plt.imshow(visualize_path(img, path))
        plt.show()
    else:
        logger.debug("No path found, still showing all paths.")
