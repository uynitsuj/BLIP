...
def get_bounding_box(loops, last_BFS_point, padding=20):
    ...
    if last_BFS_point is not None:
        ...
        dist_from_point_to_loop = [np.random.random() for i in range(len(loops))]
...
def run_inference(img_masked, img_raw, depth_masked=None, random=False, without_thresh=None,
    ...
    # print("RAW IMAGE SHAPE: ", img_raw_tensor.shape)
    ...
    path_ret, all_paths = trace(combined, right_endpoint, None, stop_when_crossing=False,
                                resume_from_edge=False, timeout=20, bboxes=loops_reformatted,
                                viz=vis_imgs)#, y_min=20, y_max=397, x_min=106, x_max=638)
    ...
    if uncertainty:
        ...
        # repurposes longest path to be a random path
        if len(all_paths) > 0:
            longest_path = all_paths[np.random.randint(0, len(all_paths))]
            if longest_path is not None and len(longest_path) > 1:
                longest_path_len = get_dist_cumsum(longest_path)[-1]
        if longest_path is None:
            longest_path_len = 1e6

    # path_ret is none at this point if tracing was uncertain in ending position or no paths reached bbox
    # longest path is none at this point if we are supposed to be tracing uncertain
    if interactive_trace:
        return (None, None, True, True, 0, 0)
    if longest_path is None: # path_ret
        logging.warning("Uncertain trace result.")
        trace_uncertain = True

    if trace_uncertain and uncertainty:
        return (None, None, True, True, 0, 0)
    elif trace_uncertain and not uncertainty:
        assert longest_path is None
        last_point = None
    else: # not trace uncertain
        last_point = np.array(longest_path[-1])

    padding_amt = 80
    bbox = get_bounding_box(loops, last_point, padding=padding_amt)
    PIXELS_TO_METERS = 668

    logger.info(f"Pull apart distance components: {bbox[3]}, {bbox[2]}, {2*padding_amt}, {longest_path_len}")
    longest_path_len = min((longest_path_len + (bbox[3] + bbox[2] - 2*padding_amt))/ PIXELS_TO_METERS / 2, 0.55)
    ...
    if not trace_uncertain:
        ...
        bbox_mask = np.zeros_like(without_thresh[:, :, 0])
        bbox_mask[int(bbox[1]) + padding_amt//2:int(bbox[1]+bbox[3]) - padding_amt//2, int(bbox[0]) + padding_amt//2:int(bbox[0]+bbox[2]) - padding_amt//2] = 1
        bbox_mask_uint8 = bbox_mask.astype(np.uint8)
        # dilated version of bbox mask
        bbox_mask_eroded = cv2.dilate(bbox_mask_uint8, np.ones((20,20), np.uint8), iterations=1)
        # find the pixels that are in the dilated bbox mask but not in the bbox mask
        pixel_to_pass = np.argwhere((bbox_mask_uint8 - bbox_mask_eroded) * img_masked[0, :, :])
        # randomly sample 1 pixel
        pixel_to_pass = pixel_to_pass[np.random.randint(len(pixel_to_pass))]
        pixels_to_pass_mask = np.zeros_like(img_masked[0])
        cs = 5
        pixels_to_pass_mask[pixel_to_pass[0]-cs:pixel_to_pass[0]+cs, pixel_to_pass[1]-cs:pixel_to_pass[1]+cs] = without_thresh[pixel_to_pass[0]-cs:pixel_to_pass[0]+cs, pixel_to_pass[1]-cs:pixel_to_pass[1]+cs, 0] > 100
        pixels_to_pass = np.argwhere(pixels_to_pass_mask)

    print("Pixels to pass", without_thresh.shape, bbox, pixels_to_pass)
    if last_point is None:
        last_point = pixel_to_pass
    if vis_imgs:
    ...
    if sample_uv_a is None:
        return None
    print("chosen points", sample_uv_a, sample_uv_b)
