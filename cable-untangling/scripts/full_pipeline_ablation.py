...
image_save_path = '/home/justin/yumi/cable-untangling/scripts/live_rollout_image_bank_uncertainty_ablation'
...
def get_endpoints(img, vis=False):
    ...
    endpoint_boxes, out_viz = loop_detectron.predict(img.color._data, thresh=0.95, endpoints=True)
    plt.clf()
    plt.imshow(out_viz)
...
def disambiguate_endpoints(endpoints, img, iface, incremental=False):
    ...
    if len(endpoints) < 2:
        ...
        iface.reveal_endpoint(img, endpoints[:, 0, :], g)
        iface.sync()
        return False
    else:
...
def run_pipeline():
    ...
    logs_file_name = f'/home/justin/yumi/cable-untangling/scripts/full_rollouts_uncertainty_ablation/logs_{date_time}.txt'
    # open logs file
    logs_file = open(logs_file_name, 'w')
    logs_file.write(f'Start time: {start_time}\n')
    action_count = 0
    vfname = f'/home/justin/yumi/cable-untangling/scripts/full_rollouts_uncertainty_ablation/vid_{date_time}.avi'
        ...
        try:
            ...
            if len(detectron_out) > 0: # detectron_out is a list of boxes
                ...
                for hulk_endpoint in endpoints:
                    ret = network_points(img, cond_grip_pose=hulk_endpoint[0], neck_point=hulk_endpoint[1], vis=vis, uncertainty=False)
                    ...
                    elif ensemble_uncertain:
                        assert False
                    elif (not trace_uncertain and not ensemble_uncertain) and (no_uncertain_type > best_uncertain_type):
                        best_ret = ret
                ...
                # just_shook = False
                if best_ret is not None and left_coords is not None:
                    ...
                    try:
                        ...
                        iface.reveal_endpoint(img, endpoints, g, closest_to_pos=((np.array(left_coords) + np.array(right_coords))/2)[::-1])
                        action_count += 1
                        continue
