...
class FullPipeline():
    ...
    def closest_valid_point(self, color, yx):
        valid_y, valid_x = np.nonzero((color[:, :] > 100))
    ...
    def get_endpoints(self, img=None):
        ...
        plt.imshow(self.img)
        plt.show()
        endpoint_boxes, out_viz = loop_detectron.predict(self.img, thresh=0.99, endpoints=True)
        print(endpoint_boxes)
        plt.clf()
        plt.imshow(out_viz)
        ...
        for box in endpoint_boxes:
            ...
            x = (xmin + xmax)/2
            y = (ymin + ymax)/2
            new_yx = self.closest_valid_point(self.img, np.array([y,x]))
